...
void clock_init(void) {
  ...
  */
  ...
  // Asius (H725 TFBGA100 with SMPS): detect early since hw_type isn't set yet.
  // Read board ID from PD4+PD5 (pulled low on asius PCB). id2 bits 0,1 == 0 means asius.
  RCC->AHB4ENR |= RCC_AHB4ENR_GPIODEN;
  __DSB();
  bool is_asius = ((GPIOD->IDR & ((1U << 4) | (1U << 5))) == 0U) &&
                  ((GPIOD->IDR & (1U << 7)) != 0U);

  // Set power mode to direct SMPS power supply (depends on the board layout)
  PackageSMPSType package_smps = is_asius ? PACKAGE_WITH_SMPS : get_package_smps_type();
